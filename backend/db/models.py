...
class GuiDeviceBinding(Base):
    """Default OpenGUI device binding for an IM user.

    Personal-assistant deployments usually have one operator and one phone,
    but storing the binding by ``platform`` + ``user_id`` keeps the mapping
    explicit and lets Weixin, Webhook, or future channels choose different
    default devices without a larger identity system.
    """

    __tablename__ = "gui_device_bindings"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    platform: Mapped[str] = mapped_column(String(32), index=True)
    user_id: Mapped[str] = mapped_column(String(256), index=True)
    opengui_base_url: Mapped[str] = mapped_column(String(1024), default="")
