...
internal class AMWxAutoReplyStep6(index: Int, helper: AMWxAutoReplyHelper) :
    ...
    override fun onExecute(data: AMDataContainer?, isResume: Boolean): AMStepCondition {
        ...
        AMEventUtils.reProcessUntilOk(
            ...
            object : Something<Boolean> {
                ...
                override fun work(timeIndex: Int): Boolean {
                    ...
                    recyclerViewNode =
                        AMNodeUtils.getFirstNodeById(rootNode, IAMWidgetWX.chatList().resourceId)
        ...
        for (i in 0..<(recyclerViewNode?.childCount ?: 0)) {
            ...
            if (node?.className.toString() == RelativeLayout::class.java.name.toString()) {
                nodeList.add(node)
            }
        }
        var contentStr = ""
        if (nodeList.isNotEmpty()) {
            ...
            val lastTextNode =
                AMNodeUtils.getFirstNodeById(lastNode, IAMWidgetWX.lastMessage().resourceId)
            if (lastTextNode != null && lastTextNode.text.toString().isNotEmpty()) {
