                            ...
                            AMNodeUtils.getFirstNodeByIdWithCallback(
                                ...
                                object :
                                    ...
                                    override fun isMatch(result: AccessibilityNodeInfo?): Boolean {
                                        return result?.text.toString() == IAMWidgetLV.marketingFirstVideos().text
                                    }
                                },
                                IAMWidgetLV.marketingFirstVideos().resourceId
                            ) ?: return false
                        return AMEventUtils.clickFirstClickableParentWithSimulate(videoNode, helper)
                    }
                }).dealWith { isSuc, intercept ->
                if (!isSuc) {
                    throw AMTaskException.business("点击AI故事成片失败")
                }
            }
        } else {
                        ...
                        val videoNode =
                            AMNodeUtils.getFirstNodeByIdWithCallback(
                                ...
                                object :
                                    MatchCallback<AccessibilityNodeInfo> {
                                    override fun isMatch(result: AccessibilityNodeInfo?): Boolean {
                                        return result?.text.toString() == IAMWidgetLV.marketingVideos().text
                                    }
                                },
                                IAMWidgetLV.marketingVideos().resourceId
                            ) ?: return false
                        return AMEventUtils.clickFirstClickableParentWithSimulate(videoNode, helper)
                    }
                }).dealWith { isSuc, intercept ->
