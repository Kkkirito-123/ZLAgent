...
import { AgentName } from "../graph-agent/config/types";
import type {
	CreateContentDto,
	OptimizeSkillDto,
	PolishContentDto,
...
export class CreatorAgentService implements OnModuleInit {
	private readonly logger = new Logger(CreatorAgentService.name);

	constructor(
		private readonly configService: ConfigService,
		private readonly platformFormatterTool: PlatformFormatterToolService,
		private readonly agentConfigProvider: AgentConfigProvider,
	...
	async *generateContent(dto: CreateContentDto): AsyncGenerator<StreamMessage> {
		...
		const systemPrompt = buildSystemPrompt(
			...
			context.tone as ContentTone | undefined,
			instructions,
		);
			...
			for await (const message of query({
							...
							for (const block of msgContent) {
								if (
									...
									if (block.type === "text" && "text" in block) {
										...
										finalContent = block.text as string;
									}
								}
							}
						}
				...
				if (message.type === "result") {
					...
					if (resultMsg.subtype === "success") {
						const successResult = resultMsg as {
							subtype: "success";
							result?: string;
						...
						inputTokens = successResult.usage?.input_tokens || 0;
						outputTokens = successResult.usage?.output_tokens || 0;
	...
	async generateContentSync(dto: CreateContentDto): Promise<ContentOutput> {
		...
		for await (const message of this.generateContent(dto)) {
			if (message.type === "final") {
				finalContent = message.content;
				inputTokens = (message.metadata?.inputTokens as number) || 0;
				outputTokens = (message.metadata?.outputTokens as number) || 0;
		...
		const metadata: ContentMetadata = {
			platform: dto.context.platform as ContentPlatform,
			scenario: dto.context.scenario as CreationScenario,
		...
		try {
			for await (const message of query({
				...
				if (message.type === "result" && message.subtype === "success") {
					const successResult = message as {
						subtype: "success";
						result?: string;
						usage?: { input_tokens?: number; output_tokens?: number };
					};
					...
					yield {
						...
						metadata: {
							inputTokens: successResult.usage?.input_tokens,
							outputTokens: successResult.usage?.output_tokens,
						},
					};
				}
			}
...
Generate a detailed research report.`;
	...
	async generateFromDescription(
		...
		this.logger.log(
			`Generating content from description: "${description.substring(0, 100)}..."`,
		);

		let systemPrompt = customSystemPrompt;
		if (!systemPrompt) {
			try {
				const config = await this.agentConfigProvider.getModelConfig(
					AgentName.CREATOR_AGENT,
				);
				systemPrompt = config.systemPrompt;
			} catch {
				this.logger.warn(
					"No active creator-agent config in DB, falling back to BASE_SYSTEM_PROMPT",
				);
		...
		if (skillIds && skillIds.length > 0) {
			const skills = await this.prisma.skill.findMany({
				...
				select: { display_name: true, content: true },
			});
			if (skills.length > 0) {
				const skillSections = skills
					.map((s) => `\n\n## Skill: ${s.display_name}\n\n${s.content}`)
					.join("");
				systemPrompt = systemPrompt + skillSections;
				this.logger.log(`Appended ${skills.length} skill(s) to system prompt`);
			}
		}
		...
		try {
			...
			for await (const message of query({
				...
				options: {
					...
					systemPrompt,
					cwd: this.skillSyncService.getCwd(),
					allowedTools: [
						"WebSearch",
						"WebFetch",
						"Read",
			...
			this.logger.log(
				`Content generated successfully (${this.countWords(finalContent)} words)`,
			);
			return finalContent;
		} catch (error) {
