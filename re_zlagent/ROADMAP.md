...
| M6 | `LOCAL` | Add long-task plans, projections, and restart context packs. |
| M7 | `LOCAL` | Add the long-task ledger storage boundary. |
| M8 | `LOCAL` | Add SQLite durability for interactions, artifacts, and side effects. |
| M9 | `LOCAL` | Add read-only parked-run classification. |
| M10 | `LOCAL` | Add conservative DAG execution assessment. |
| M11 | `LOCAL` | Record runtime side-effect results in the ledger. |
| M12 | `LOCAL` | Add the composite long-task progress snapshot. |
| M13 | `LOCAL` | Close acceptance trust and immutable task-contract semantics. |
| M14 | `LOCAL` | Persist plans and continue the complete unfinished frontier after recovery. |
| M15 | `LOCAL` | Make external side effects crash-safe through an outbox boundary. |
| M16 | `LOCAL` | Add durable worker claiming, leases, heartbeat, and retry budgets. |
| M17 | `LOCAL` | Deliver a real task submission and execution MVP. |
| M18 | `LOCAL` | Add CI, benchmarks, fault injection, and latency/reliability gates. |
...
| M21 | `NEXT` | Close migration and delete legacy source only after explicit approval. |
...
smoke checks, and package dry-run; the workflow template runs the same command on
Python 3.11 and 3.13. Because repository rules currently forbid writes outside
`re_zlagent/`, remote workflow activation remains an explicit M21 repository-root
operation rather than a hidden parent-tree change.
...
**Recommended repository-root closure (not yet approved):**

1. Review the dirty legacy worktree before staging anything. Preserve intentional
   source and documentation changes in a dedicated snapshot commit or archive
   branch approved by the user; never capture credentials or runtime data merely
   to make the worktree clean.
2. Commit the complete validated `re_zlagent/` tree and create an annotated
   `pre-rebuild-root-promotion` tag after the legacy snapshot is preserved. The
   tag must identify a commit that can recover both the reviewed legacy state and
   the validated rebuild. Git actions require explicit user approval.
3. Promote the contents of `re_zlagent/` to the repository root so package paths,
   README, license, `.gitignore`, and `.github/workflows/quality.yml` become the
   canonical project instead of leaving a permanent nested wrapper.
4. Remove tracked legacy product material in one reviewable change: `backend/`,
   `OpenGUI-main/`, old root `tests/`, `config/`, `scripts/`, Docker/start-stop
   wrappers, old runtime documentation, old requirements, and old media assets.
5. Do not automatically delete or commit `.env`, `workspace/`, database files,
   logs, credentials, or other user runtime data. Their retention or secure
   deletion is a separate explicit data decision.
6. Run the release gate from the promoted root, test the installed console
   scripts, and verify the GitHub workflow path. A clean checkout must reproduce
   the same result.
7. Roll back code by checking out the annotated pre-promotion tag. Runtime data
   rollback is outside Git and requires a separately verified backup.

**Audited repository footprint (2026-07-11):** Git currently tracks 2,338 files:
...
legacy product change therefore affects 2,208 tracked files, while `workspace/`
is retained by default and the rebuilt root replaces selected root metadata.
Outside `re_zlagent/`, the worktree also contains 31 modified files, 3 tracked
deletions, and 4 untracked paths. Those 38 dirty entries are a hard preservation
gate: no deletion or root promotion may start until the user reviews how they are
snapshotted or retained.

Keeping `re_zlagent/` nested is possible, but it leaves parent-level wrappers and
prevents its contained GitHub workflow from running. Root promotion is therefore
the recommended closure layout.
...
**Deletion decision gate:** the user must choose whether the target is intentionally
local-CLI-only or name the deferred product capabilities that are required before
legacy deletion. Until that decision, deleting `backend/`, root launch scripts,
requirements, concrete gateway configuration, or `OpenGUI-main/` would be a
functional product regression even though the new core harness is independent.
...
- Do not delete old source before M21, even when individual replacement modules
  appear complete.
