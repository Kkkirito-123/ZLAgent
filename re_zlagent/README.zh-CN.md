...
`re_zlagent` 是 ZLAgent 的新重构工作区。

目标是先把 ZLAgent 重建成一个可复用、可验证、可恢复的 agent harness，再逐步接入应用层、网关层和具体产品能力。`re_zlagent/` 外的旧源码默认只作为参考，除非明确批准某个迁移切片。
...
当前 `re_zlagent` 还不是旧 `backend/` 的完整替代。完整阶段历史、能力决策和验收门槛统一记录在用户版 [`ROADMAP.zh-CN.md`](./ROADMAP.zh-CN.md)；AI 使用英文 [`ROADMAP.md`](./ROADMAP.md)。

```text
M1-M5   LANDED   已提交的基础和恢复边界
M6-M12  LOCAL    长任务投影、ledger、存储、scanner 和进度
M13     LOCAL    验收信任与不可变任务事实
M14     LOCAL    持久化计划与完整恢复继续执行
M15     LOCAL    崩溃安全 side-effect outbox
M16     LOCAL    持久 worker 所有权和重试预算
M17     LOCAL    真实任务提交与执行 MVP
M18     LOCAL    可靠性和发布门槛
M19-M20 DEFERRED 可选迁移和安全 DAG 并发
M21     NEXT     最终迁移关闭和旧源码删除门槛
```

本地产品 MVP 已支持持久化提交、worker 执行、批准恢复，以及跨进程重启读取
已验证结果。M18 已增加版本化 6-case release corpus、语义和时延阈值、Ruff、
mypy 和统一机器可读质量命令。M21 把 `re_zlagent` 提升为仓库根时，目录内 workflow
才会成为远端阻断器。

M21 门槛通过并获得用户明确批准之前，禁止删除旧源码。
...
```bash
PYTHONPATH=re_zlagent/src python -m re_zlagent.check --pretty
```

## 下一步

M21 是唯一允许开始的下一阶段：审计全部旧能力、确认新源码不再 import 旧
backend、激活仓库根 CI、建立 rollback 证据，并在任何删除前请求用户明确批准。
进入条件和删除门槛见 [`ROADMAP.zh-CN.md`](./ROADMAP.zh-CN.md)。

Wiki、Graph-RAG 和 geo 已从当前目标中移除。MCP、cron、OpenGUI 和 DAG 并发明确延后。

## 重要边界

不要删除旧源码，直到 capability ledger 明确说明每个旧能力已经被替换、延期、明确移除或不再需要，并且用户明确确认删除风险。
