...
`re_zlagent/` 是新的 ZLAgent 重构工作区。它不是旧 `backend/`，也不是 `OpenGUI-main/` 的 fork。

父级仓库中的内容默认作为参考：

- `backend/`：旧 ZLAgent 源码，默认只读。
- `tests/`：旧测试材料，默认只读。
- `OpenGUI-main/`：OpenGUI 结构和移动端 agent runtime 的参考。
- `zlagent-governance-lab/`：历史草稿材料，不是当前事实源。
...
删除旧源码前，必须通过英文 `ROADMAP.md` 的最终迁移门槛、完成 capability ledger 检查并获得用户明确确认。不能因为 `re_zlagent` 中某个基础能力已实现，就直接删除旧 `backend/`。
